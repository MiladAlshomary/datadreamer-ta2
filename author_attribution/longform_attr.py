...
def apply_srs(input_dir, output_dir, run_id):
    ...
    with tf.device('/GPU:1'):
        ...
        for i in range(5):
            X_train, X_val, Y_train, Y_val, _ = split_tokenize(X=X, Y=Y, X_test=X_test, max_len=max_len)
            model.fit(
        ...
        X_test = _
        results = model.predict(x ={'input_ids':X_test['input_ids'],'attention_mask':X_test['attention_mask']}, batch_size=16)
        ...
        for i, row in candidates.iterrows():
            ...
            predictions[row["authorSetIDs"]] = np.add(predictions[row["authorSetIDs"]], results[i])
        ...
        save_output_files(prefix, output_dir, run_id, output_matrix, authorIDs, authorSetIDs)

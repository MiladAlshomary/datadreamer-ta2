...
class EER_Evaluator(SentenceEvaluator):
    ...
    def __init__(
        ...
        self.eer_prefix = eer_prefix

        self.with_filtering = with_filtering
        self.mask = mask
        self.as_predictor= as_predictor
        
        self.csv_file = f"{self.eer_prefix}_eer_results.csv"
        self.csv_headers = [
            "epoch",
            "steps",
            "eer",
...
class PAUSIT_MODEL:
    ...
    def build_luar_based_model(self):
        ...
        style_model = LuarTransformer("../training_source/rrivera1849/LUAR-MUD", tokenizer_args = {'trust_remote_code':True}, model_args = {'trust_remote_code':True})
    ...
    def train(self, training_epochs=3, luar_based=False, evaluation_steps=100, valid_from_training=False, batch_size=32, eer_prefix=None):
        ...
        if valid_from_training:
            ...
            # eer_train_evaluator = EER_Evaluator(train_query_df, train_candidate_df, gt_assignment_training, q_author_training, c_author_training, batch_size=batch_size, output_path="/data/araghavan/HIATUS/datadreamer-ta2/output/logs/train", eer_prefix=eer_prefix)
            eer_train_evaluator = EER_Evaluator(train_query_df, train_candidate_df, gt_assignment_training, q_author_training, c_author_training, batch_size=batch_size, output_path=os.path.dirname(self.output_dir), eer_prefix=f"train_{eer_prefix}")
            eer_valid_evaluator = EER_Evaluator(valid_query_df, valid_candidate_df, 
                # gt_assignment_valid, q_author_valid, c_author_valid, batch_size=batch_size, output_path="/data/araghavan/HIATUS/datadreamer-ta2/output/logs/dev", eer_prefix=eer_prefix)
                gt_assignment_valid, q_author_valid, c_author_valid, batch_size=batch_size, output_path=os.path.dirname(self.output_dir), eer_prefix=f"dev_{eer_prefix}")

            print('Train EER before training', self.model.evaluate(eer_train_evaluator))
            ...
            eer_test_evaluator = EER_Evaluator(self.query_df, self.candidates_df, 
                # ground_truth_assignment, query_authors, candidate_authors, batch_size=batch_size, output_path="/data/araghavan/HIATUS/datadreamer-ta2/output/logs/dev", eer_prefix=eer_prefix)
                ground_truth_assignment, query_authors, candidate_authors, batch_size=batch_size, output_path=os.path.dirname(self.output_dir), eer_prefix=f"dev_{eer_prefix}")
        ...
        if eer_test_evaluator:
            print('EER after training', self.model.evaluate(eer_test_evaluator))
        else:
            print('EER after training', self.model.evaluate(eer_valid_evaluator))


    def predict_and_evaluate(self, output_path, input_path=None, batch_size=16, with_filtering=False, filter_prcnt=0.5, eer_prefix=None):
        ...
        if input_path != None:
            ...
            # Dynamically find the prefix from the filenames
            files = os.listdir(ground_truth_path)
            prefix = None
            for file in files:
                if file.endswith('_TA2_groundtruth.npy'):
                    prefix = file.split('_TA2_groundtruth.npy')[0]
                    break

            if not prefix:
                raise FileNotFoundError("Ground truth file with '_TA2_groundtruth.npy' suffix not found in the directory.")
            ...
            ground_truth_assignment = np.load(open(os.path.join(ground_truth_path, f'{prefix}_TA2_groundtruth.npy'), 'rb'))

            # Load candidate authors
            with open(os.path.join(ground_truth_path, f'{prefix}_TA2_candidate-labels.txt'), 'r') as f:
                candidate_authors = [a[2:-3] for a in f.read().split('\n') if a]

            # Load query authors
            with open(os.path.join(ground_truth_path, f'{prefix}_TA2_query-labels.txt'), 'r') as f:
                query_authors = [a[2:-3] for a in f.read().split('\n') if a]
        ...
        eer_evaluator = EER_Evaluator(query_df, candidates_df, 
            # ground_truth_assignment, query_authors, candidate_authors, batch_size=batch_size, as_predictor=True, with_filtering=with_filtering, output_path="/data/araghavan/HIATUS/datadreamer-ta2/output/logs/test", eer_prefix=eer_prefix)#, mask=self.mask)
            ground_truth_assignment, query_authors, candidate_authors, batch_size=batch_size, as_predictor=True, with_filtering=with_filtering, output_path=os.path.dirname(output_path), eer_prefix=f"test_{eer_prefix}")#, mask=self.mask)

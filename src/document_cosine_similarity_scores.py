...
# def calculate_cosine_similarity_pairwise(input_file, output_file, chunk_size=100):
#     """
#     Optimized pairwise cosine similarity computation for large files.
...
#     """
#     print("Called calculate_cosine_similarity_pairwise")
...
#         total_lines = sum(1 for _ in open(input_file, 'r'))  # Count total lines for progress bar
...
#         print("Pre computed starting offset for each line")

#         infile.seek(0)  # Reset file for actual processing
        
#         # Process each document
#         for i, start_offset in enumerate(tqdm(start_offsets, total=total_lines, desc="Processing documents")):
#             infile.seek(start_offset)
#             current_doc = json.loads(infile.readline())
#             current_id = current_doc['documentID']
#             current_embedding = np.array(current_doc['embeddings']).reshape(1, -1)
...
#                 chunk_embeddings = np.array([doc['embeddings'] for doc in chunk])
#                 chunk_ids = [doc['documentID'] for doc in chunk]

#                 # Compute cosine similarities
#                 similarities = cosine_similarity(current_embedding, chunk_embeddings).flatten()
...
#     # # Input file containing document embeddings and IDs
#     # input_file = "../data/sadiri_processed_with_embeddings_wo_ao3_subset_10.jsonl"

#     # # Output file to save pairwise cosine similarity data
#     # output_file = "../data/dpcs_sadiri_processed_with_embeddings_wo_ao3_subset_10.jsonl"

#     # Chunk size for processing
#     chunk_size = 2048

#     # Calculate and save pairwise cosine similarity
#     calculate_cosine_similarity_pairwise(input_file, output_file, chunk_size=chunk_size)
